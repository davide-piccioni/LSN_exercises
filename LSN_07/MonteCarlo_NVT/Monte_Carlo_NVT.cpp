...
{ 
  ...
  for(int i=1; i <= nstep*5; ++i) //Simulation
  {
    Move();
    if(i%(nstep)==0){
      Measure();
      cout << "Potential energy after "<<i<<" steps = " << walker[iv]<< endl;
    }
  }
  cout<<endl;
  
  //Stampa dati istantanei
  ofstream file_out_pot;
  file_out_pot.open("Inst_potential.dat");
  
  ofstream file_out_press;
  file_out_press.open("Inst_pre.dat");
  ...
  for(int iblk=1; iblk <= nblk; ++iblk) //Simulation
  {
    ...
    for(int istep=1; istep <= nstep; ++istep)
    {
      Move();
      Measure();
      file_out_pot<<walker[iv]<<endl;
      file_out_press<<rho/beta+walker[iw]/(3*vol)<<endl;
...
//Read input informations
  ...
  cout << "The program performs Metropolis moves with uniform translations" << endl; //Cosa significa uniformi?
...
//update of the histogram of g(r)
     ...
     if(dr<box/2){
       ...
       walker[int(dr/bin_size)+igofr]=walker[int(dr/bin_size)+igofr]+2;
     }
...
void Averages(int iblk) //Print results for current block
{
  ...
  for (int k=igofr; k<igofr+nbins;k++){
    ...
    double V_r_dr=4*M_PI/3*( pow((k-igofr+1)*bin_size,3)-pow((k-igofr)*bin_size,3) );
    Gofr  << iblk <<  "   " <<(k-igofr)*bin_size<<"   " << (blk_av[k]/blk_norm)/(m_part*rho*V_r_dr) << "   " << (glob_av[k]/(double)iblk)/(m_part*rho*V_r_dr) << "   " << (err_gdir)/(m_part*rho*V_r_dr) << endl;
  }
