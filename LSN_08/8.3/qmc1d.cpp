...
{
  //double k_elastic = 1;
  //return k_elastic*val*val/2.0; //Armonic Oscillator
...
{
  //double k_elastic =1;
  //return k_elastic*val; //Armonic Oscillator
...
{
  //double k_elastic=1;
  //return k_elastic; //Armonic Oscillator
  return 12*val*val-5;
}
...
{
  //return 1.0; //Constant
  //return exp(-0.5*x*x); //Armonic Oscillator
  double mu=0.79;
  double sigma=0.62;
...
{
  //return 0; //Constant
  //return x*x*exp(-0.5*x*x) - exp(-0.5*x*x); //Armonic Oscillator
  double mu=0.79;
  double sigma=0.62;
...
void finalizeTotalEnergyEstimator()
{
	...
	for(int i=0;i<timeslices;i++)
	{
	  ...
	  double kinetic_energy_square_avg = kinetic_energy_square_accumulator[i]/blocks;
	  double k_error =sqrt(abs(kinetic_energy_average*kinetic_energy_average-kinetic_energy_square_avg)/blocks);
	  out<<i<<" "<<kinetic_energy_average+potential_energy_average<<" "<<sqrt(k_error*k_error+p_error*p_error)<<endl;
